...
public:
	 ...
	 */
	virtual void on_event(SimulationEvent event, const ProgressInfo& info, const std::string& message = {}) = 0;

	/**
	 * @brief Called for generic progress updates
	 * @param info Current progress information
	 */
	virtual void on_progress(const ProgressInfo& info) { on_event(SimulationEvent::Progress, info); }
...
public:
	explicit ConsoleObserver(bool verbose = false,
	...
	void on_event(SimulationEvent event, const ProgressInfo& info, const std::string& message = {}) override {
		...
		switch (event) {
			...
			case SimulationEvent::StageChanged: std::cout << "Stage: " << info.current_stage << std::endl; break;

			case SimulationEvent::Completed:
				std::cout << "Simulation completed in " << info.elapsed_time << " seconds\n";
				break;

			case SimulationEvent::Error: std::cout << "Error: " << message << std::endl; break;

			case SimulationEvent::Paused: std::cout << "Simulation paused\n"; break;

			case SimulationEvent::Resumed: std::cout << "Simulation resumed\n"; break;
		}
	}

private:
	void print_progress(const ProgressInfo& info) {
		std::cout << "Progress: " << info.current_photon << "/" << info.total_photons << " (" << std::fixed
				  << std::setprecision(1) << info.progress_percent << "%) ";
...
public:
	...
	void on_event(SimulationEvent event, const ProgressInfo& info, const std::string& message = {}) override {
		...
		switch (event) {
			case SimulationEvent::Started: log_file_ << "STARTED: " << info.total_photons << " photons"; break;
			case SimulationEvent::Progress:
				log_file_ << "PROGRESS: " << info.current_photon << "/" << info.total_photons << " ("
						  << info.progress_percent << "%)";
				break;
			case SimulationEvent::Completed: log_file_ << "COMPLETED: " << info.elapsed_time << "s"; break;
			case SimulationEvent::Error: log_file_ << "ERROR: " << message; break;
			default:
...
public:
	 ...
	 */
	void add_observer(std::shared_ptr<SimulationObserver> observer) { observers_.push_back(std::move(observer)); }
	 ...
	 */
	void set_stage(const std::string& stage) { current_progress_.current_stage = stage; }

	/**
	 * @brief Get current progress info
	 */
	const ProgressInfo& progress() const { return current_progress_; }
...
private:
	void update_progress_timing() {
		...
		if (current_progress_.current_photon > 0 && current_progress_.total_photons > 0) {
			...
			double remaining_photons =
				static_cast<double>(current_progress_.total_photons - current_progress_.current_photon);

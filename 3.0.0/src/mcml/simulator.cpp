...
Simulator::Simulator(std::string in_file) :
	m_mci {in_file}, m_params {}, m_reader {std::make_shared<Reader>(in_file)},
	m_cin_reader {std::make_shared<ReaderCin>()}, m_writer {std::make_shared<Writer>()},
	m_timer {std::make_shared<Timer>()}, m_random {std::make_shared<Random>()},
...
/**
 * @brief Add an observer to receive simulation progress updates
 * @param observer Shared pointer to simulation observer
 */
...
/**
 * @brief Remove an observer from receiving simulation progress updates
 * @param observer Shared pointer to simulation observer to remove
 */
...
/**
 * @brief Get current simulation progress information
 * @return Reference to progress information structure
 */
...
/**
 * @brief Run simulation with input file parameters
 * Reads input file and executes the specified number of simulation runs
 */
void Simulator::simulate() {
	bool retry = false;
	do {
		retry = !prompt_filename(m_mci);
		if (m_mci == "Q") {
			return;
		}
	}
	...
	m_reader->read_params(*m_reader, m_params);

	std::size_t run_i = 1;
	while (run_i <= m_params.num_runs) {
		m_reader->read_run_params(*m_reader, m_params);
...
/**
 * @brief Resume simulation from previous output file
 * Loads previous simulation results and continues with additional photons
 */
void Simulator::resume() {
	...
	bool retry = false;
	do {
		retry = prompt_filename(filename, ".mco");
		if (filename == "Q") {
			return;
		}
	}
	...
	try {
		...
		file_reader->skip_line(*file_reader);
		file_reader->read_mediums(*file_reader, m_params.mediums);

		if (m_params.mediums.empty()) {
			throw std::runtime_error("No mediums found in file.");
		}

		file_reader->read_run_params(*file_reader, m_params);

		// Add photons / time limit
		m_cin_reader->read_target(*m_cin_reader, m_params, m_params.target, true);

		Radiance radiance;
		file_reader->read_radiance(*file_reader, m_params, m_random, radiance);
		m_tracer = std::make_shared<Tracer>(m_params, m_random, radiance);

		scale_reflectance(*m_tracer, ScaleMode::Unscale);
		scale_transmittance(*m_tracer, ScaleMode::Unscale);
		scale_absorption(*m_tracer, ScaleMode::Unscale);
...
/**
 * @brief Run simulation in interactive mode
 * Prompts user for simulation parameters and executes simulation
 */
void Simulator::interactive() {
	// Read params interactively
	m_cin_reader->read_params(*m_cin_reader, m_params);
	...
	if (std::toupper(c) == 'Y') {
		...
		if (!mci_file.is_open()) {
			...
			m_writer->write_params(mci_file, m_params);
		}
	}
...
/**
 * @brief Interactive edit mode for simulation parameters
 * Loads input file and allows user to modify parameters interactively
 */
void Simulator::interactive_edit() {
	// Read mci file
	bool retry = false;
	std::string filename;

	do {
		retry = !prompt_filename(filename);
...
void Simulator::run(std::size_t run_index, bool start_new) {
	...
	report_target(m_params.num_runs - run_index);
	...
	do {
		...
		switch (m_params.target.control_bit) {
			...
			case ControlBit::Both: {
				exit =
					(photon_traced > m_params.target.photons_limit) || (m_timer->punch() >= m_params.target.time_limit);
				break;
			}
		}
	}
...
bool Simulator::interactive_run() {
	char command {'\0'};
	std::string string;
	bool start_sim;
	...
	while (cont) {
		...
		if (std::toupper(command) == 'Y') {
			...
			m_writer->write_params(file, m_params);
		}

		// Quit change menu and start simulation
		if (start_sim && !validate_params()) {
...
bool Simulator::edit_menu(char command) {
	switch (std::toupper(command)) {
		case 'O': {
			m_writer->write_params(std::cout, m_params);
			break;
		}
		case 'M': {
			edit_mediums();
		...
		case 'D': {
			edit_grid_spacing();
			break;
		} // Change dz, dr, dt only
		case 'N': {
			edit_grid_size();
			break;
		} // Change nz, nr, nt, na only
		case 'G': {
			edit_grid();
			break;
		} // Keep original combined grid editor
		case 'C': {
			edit_record();
			break;
		}
...
bool Simulator::validate_params() {
	...
	if (!m_params.source.medium_name.empty()) { // TODO: write into a function
		if (m_params.layers[m_params.source.layer_index].name == m_params.source.medium_name) {
			if ((std::fabs(m_params.source.z - m_params.layers[m_params.source.layer_index].z1)
...
/**
 * @brief Report simulation run start and target parameters
 * @param runs_remaining Number of runs left to execute
 */
void Simulator::report_target(std::size_t runs_remaining) {
	using namespace std::chrono;
...
/**
 * @brief Report simulation progress during execution
 * @param photons_done Number of photons processed so far
 */
void Simulator::report_progress(std::size_t photons_done) {
	if (m_params.target.control_bit == ControlBit::TimeLimit) {
		...
		std::cout << std::format("{:>12} ({:6.2f}%)\t", photons_done,
								 static_cast<float>(photons_done) * 100.0f
									 / static_cast<float>(m_params.target.photons_limit))
				  << std::endl;
	}
}

/**
 * @brief Report final simulation results and timing information
 */
void Simulator::report_result() {
	...
	// Scale results
	Radiance& radiance = *m_tracer;
	scale_reflectance(radiance);
	scale_transmittance(radiance);
	scale_absorption(radiance);

	std::fstream file(m_params.output_filename, std::ios::out);

	if (!file.is_open()) {
		m_observer_subject.notify_event(mcml::SimulationEvent::Error,
										"Cannot open output file to write: " + m_params.output_filename);
		std::cout << "Can not open output file to write." << std::endl;
		std::exit(1);
	}

	// Write results to file
	m_writer->write_results(file, m_params, *m_tracer, m_random);
}

bool Simulator::prompt_filename(std::string& result, std::string file_type) {
...
void Simulator::edit_mediums() {
	std::cout << "Current medium list: " << std::endl;
	m_writer->write_mediums(*m_writer, m_params);
	std::cout << std::endl;

	if (prompt_edit()) {
		m_params.mediums.clear();
		m_cin_reader->read_mediums(*m_cin_reader, m_params.mediums);
	}
}

void Simulator::edit_output() {
	std::cout << "Current output file name and format: " << std::endl;
	m_writer->write_filename(*m_writer, m_params);
	std::cout << std::endl;
	...
	if (!m_cin_reader->read_output(*m_cin_reader, m_params.output_filename)) {
		return;
	}
}

void Simulator::edit_grid() {
	std::cout << "Current dz, dr, dt: " << std::endl;
	m_writer->write_grid_params(*m_writer, m_params);
	std::cout << std::endl;
	...
	m_cin_reader->read_grid(*m_cin_reader, m_params.grid);
}

void Simulator::edit_grid_spacing() {
	std::cout << "Current dz, dr, dt: " << std::endl;
	m_writer->write_grid_params(*m_writer, m_params);
	std::cout << std::endl;
...
void Simulator::edit_record() {
	m_writer->write_record(*m_writer, m_params);
	std::cout << std::endl;

	if (prompt_edit()) {
		m_cin_reader->read_record(*m_cin_reader, m_params, m_params.record);
	}
}

void Simulator::edit_weight() {
	std::cout << "Current threshold weight: " << std::endl;
	m_writer->write_weight(*m_writer, m_params);
	std::cout << std::endl;

	m_cin_reader->read_weight(*m_cin_reader, m_params.weight_threshold);
}

void Simulator::edit_layers() {
	std::cout << "Current layer specification: " << std::endl;
	m_writer->write_layers(*m_writer, m_params);
	std::cout << std::endl;

	if (prompt_edit()) {
		m_cin_reader->read_layers(*m_cin_reader, m_params, m_params.layers);
		m_params.num_layers = m_params.layers.size() - 2;
	}
}

void Simulator::edit_target() {
	std::cout << "Current value: " << std::endl;
	m_writer->write_end_criteria(*m_writer, m_params);
	std::cout << std::endl;

	m_cin_reader->read_target(*m_cin_reader, m_params, m_params.target);
}

void Simulator::edit_source() {
	std::cout << "Current source type: " << std::endl;
	m_writer->write_source_type(*m_writer, m_params);
	std::cout << std::endl;
	...
	m_writer->write_photon_source(*m_writer, m_params);
	std::cout << std::endl;
	...
	if (!m_cin_reader->read_source(*m_cin_reader, m_params, m_params.source)) {
		return;
	}
}

void Simulator::show_edit_menu_help() {
...
void Simulator::edit_random_seed() {
...
void Simulator::edit_source_position() {
	std::cout << "Current starting position: " << std::endl;
	m_writer->write_photon_source(*m_writer, m_params);
	std::cout << std::endl;
...
void Simulator::scale_reflectance(Radiance& radiance, ScaleMode mode) {
	/*
	...
	auto op = (mode == ScaleMode::Scale) ? [](double x, double y) { return x * y; }
	...
	if (mode == ScaleMode::Scale) {
		...
		radiance.R_error = (scale1 * radiance.R_error) * (scale1 * radiance.R_error)
						   + 1 / scale1 * radiance.R_total * radiance.R_total;
		radiance.Rb_error = (scale1 * radiance.Rb_error) * (scale1 * radiance.Rb_error)
							+ 1 / scale1 * radiance.Rb_total * radiance.Rb_total;
	}
	...
	scale1 =
		2.0 * std::numbers::pi * dr * dr * std::numbers::pi * da * static_cast<double>(m_params.target.photons_limit);
	if (m_params.record.R_ra) {
		for (std::size_t ir = 0; ir < nr; ir++) {
			for (std::size_t ia = 0; ia < na; ia++) {
				double scale2 =
					1.0 / ((static_cast<double>(ir) + 0.5) * sin(2.0 * (static_cast<double>(ia) + 0.5) * da) * scale1);
...
void Simulator::scale_transmittance(Radiance& radiance, ScaleMode mode) {
	/*
	 * a is the angle alpha.
	 *
	 * Scale Rd(r,a) and Td(r,a) by:
	...
	if (mode == ScaleMode::Scale) {
		...
		radiance.T_error = (scale1 * radiance.T_error) * (scale1 * radiance.T_error)
						   + 1 / scale1 * radiance.T_total * radiance.T_total;
		radiance.Tb_error = (scale1 * radiance.Tb_error) * (scale1 * radiance.Tb_error)
							+ 1 / scale1 * radiance.Tb_total * radiance.Tb_total;
	}
		...
		for (std::size_t ir = 0; ir < nr; ir++) {
			for (std::size_t ia = 0; ia < na; ia++) {
				for (std::size_t it = 0; it < nt; it++) {
					double scale2 =
						1.0
						/ ((static_cast<double>(ir) + 0.5) * sin(2.0 * (static_cast<double>(ia) + 0.5) * da) * scale1);
...
void Simulator::scale_absorption(Radiance& radiance, ScaleMode mode) {
	auto op = (mode == ScaleMode::Scale) ? [](double x, double y) { return x / y; }
										 : [](double x, double y) { return x * y; };
	...
	// unscale A
		...
		radiance.A_total *= scale1;
		radiance.A_error = (scale1 * radiance.A_error) * (scale1 * radiance.A_error)
						   + 1 / scale1 * radiance.A_total * radiance.A_total;
	}
	...
	scale1 = 2.0 * std::numbers::pi * dr * dr * dz * static_cast<double>(m_params.target.photons_limit);
	if (m_params.record.A_rz) {
		for (std::size_t ir = 0; ir < nr; ir++) {
			for (std::size_t iz = 0; iz < nz; iz++) {
				radiance.A_rz[ir][iz] = op(radiance.A_rz[ir][iz], (static_cast<double>(ir) + 0.5) * scale1);
			}
		}
	}
	...
	if (m_params.record.A_rzt) {
		for (std::size_t ir = 0; ir < nr; ir++) {
			for (std::size_t iz = 0; iz < nz; iz++) {
				for (std::size_t it = 0; it < nt; it++) {
					radiance.A_rzt[ir][iz][it] =
						op(radiance.A_rzt[ir][iz][it], (static_cast<double>(ir) + 0.5) * scale2);
				}
			}
		}
	}

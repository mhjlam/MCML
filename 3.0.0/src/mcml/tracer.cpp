...
Tracer::Tracer(RunParams& params, std::shared_ptr<Random>& random) :
	...
	auto alloc2 = [](std::size_t x, std::size_t y) { return vec2<double>(x, vec1<double>(y, 0.0)); };

	auto alloc1 = [](std::size_t x) { return vec1<double>(x, 0.0); };
	...
	if (m_params.record.R_rat) {
		m_radiance.R_rat = alloc3(nr, na, nt);
	}
	if (m_params.record.R_ra) {
		m_radiance.R_ra = alloc2(nr, na);
	}
	if (m_params.record.R_rt) {
		m_radiance.R_rt = alloc2(nr, nt);
	}
	if (m_params.record.R_at) {
		m_radiance.R_at = alloc2(na, nt);
	}
	if (m_params.record.R_r) {
		m_radiance.R_r = alloc1(nr);
	}
	if (m_params.record.R_a) {
		m_radiance.R_a = alloc1(na);
	}
	if (m_params.record.R_t) {
		m_radiance.R_t = alloc1(nt);
	}

	// Transmittance
	if (m_params.record.T_rat) {
		m_radiance.T_rat = alloc3(nr, na, nt);
	}
	if (m_params.record.T_ra) {
		m_radiance.T_ra = alloc2(nr, na);
	}
	if (m_params.record.T_rt) {
		m_radiance.T_rt = alloc2(nr, nt);
	}
	if (m_params.record.T_at) {
		m_radiance.T_at = alloc2(na, nt);
	}
	if (m_params.record.T_r) {
		m_radiance.T_r = alloc1(nr);
	}
	if (m_params.record.T_a) {
		m_radiance.T_a = alloc1(na);
	}
	if (m_params.record.T_t) {
		m_radiance.T_t = alloc1(nt);
	}

	// Absorption
	if (m_params.record.A_rzt) {
		m_radiance.A_rzt = alloc3(nr, nz, nt);
		m_radiance.Ab_zt = alloc2(nz, nt);
	}
	if (m_params.record.A_rz) {
		m_radiance.A_rz = alloc2(nr, nz);
		m_radiance.Ab_z = alloc1(nz);
	}
	if (m_params.record.A_zt) {
		m_radiance.A_zt = alloc2(nz, nt);
	}
	if (m_params.record.A_z) {
		m_radiance.A_z = alloc1(nz);
	}
	if (m_params.record.A_t) {
		m_radiance.A_t = alloc1(nt);
	}
}
...
void Tracer::spin(Photon& photon, double g) {
	...
	double sin_psi =
		(psi < std::numbers::pi) ? std::sqrt(1.0 - cos_psi * cos_psi) : -std::sqrt(1.0 - cos_psi * cos_psi);
...
double Tracer::fresnel(double eta_i, double eta_t, double cos_ai, double& cos_at) {
	...
	// General incidence
		...
		// Arranged for speed
		return (0.5 * sin_am * sin_am) * (cos_am * cos_am + cos_ap * cos_ap) / (sin_ap * sin_ap * cos_am * cos_am);
	}
}

void Tracer::record_reflectance(Photon& photon, double reflectance) {
	double x = photon.position.x;
	double y = photon.position.y;
...
void Tracer::record_transmittance(Photon& photon, double reflectance) {
	double x = photon.position.x;
	double y = photon.position.y;
...
void Tracer::cross_up(Photon& photon) {
	...
	if (PARTIAL_REFLECTION) {
		...
		if (layer == 1 && refl < 1.0) {
			...
			record_reflectance(photon, refl);
...
void Tracer::cross_down(Photon& photon) {
	// Z directional cosine
	double uz = photon.direction.z;
	...
	if (PARTIAL_REFLECTION) {
		if (layer == m_params.num_layers && refl < 1.0) {
			photon.direction.z = uz1;
			record_transmittance(photon, refl);
			photon.direction.z = -uz;
		}
		// Transmitted to current_layer + 1
		else if (m_random->next() > refl) {
		...
		if (m_random->next() > refl) {
			if (layer == m_params.num_layers) {
				photon.direction.z = uz1;
				record_transmittance(photon, 0.0);
...
void Tracer::hop_drop_spin(Photon& photon) {
	Layer& layer = m_params.layers[photon.current_layer];

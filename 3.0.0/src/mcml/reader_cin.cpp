...
bool ReaderCin::read_params([[maybe_unused]] std::istream& input, RunParams& params) {
	if (!read_mediums(*m_input, params.mediums)) {
		std::cerr << "Error: Failed to read mediums." << std::endl;
		return false;
	}

	if (!read_output(*m_input, params.output_filename)) {
		std::cerr << "Error: Failed to read output filename." << std::endl;
		return false;
	}

	if (!read_layers(*m_input, params, params.layers)) {
		std::cerr << "Error: Failed to read layers." << std::endl;
		return false;
	}

	if (!read_source(*m_input, params, params.source)) {
		std::cerr << "Error: Failed to read source." << std::endl;
		return false;
	}

	if (!read_grid(*m_input, params.grid)) {
		std::cerr << "Error: Failed to read grid parameters." << std::endl;
		return false;
	}

	if (!read_record(*m_input, params, params.record)) {
		std::cerr << "Error: Failed to read record." << std::endl;
		return false;
	}

	if (!read_target(*m_input, params, params.target)) {
		std::cerr << "Error: Failed to read target." << std::endl;
		return false;
	}

	if (!read_weight(*m_input, params.weight_threshold)) {
...
bool ReaderCin::read_mediums(std::istream& in, vec1<Layer>& out) {
	std::string prompt = "Specify number of mediums (>= 1)";
	std::string error = "Invalid number of mediums";

	auto [success, num_media] =
		read_in_single<int>(in, prompt, error, false, [](const int& num_media) { return num_media >= 1; });
	if (!success) {
		return false;
	}
	...
	for (std::size_t i = 0; i < static_cast<std::size_t>(num_media); i++) {
		...
		auto [s1, name] = read_in_single<std::string>(
			in, "  Name of medium", "Invalid medium name or duplicate", false, [&mediums](const std::string& name) {
		...
		auto [s2, eta] = read_in_single<double>(in, "  Refractive index n (>= 1.0): ", {}, false,
												[&mediums](const double& value) { return value >= 1.0; });
		if (!s2) {
			return false;
		}

		auto [s3, mua] = read_in_single<double>(in, "  Absorption coefficient mua (>= 0.0 /cm): ", {}, false,
												[&mediums](const double& value) { return value >= 0.0; });
		if (!s3) {
			return false;
		}

		auto [s4, mus] = read_in_single<double>(in, "  Scattering coefficient mus (>= 0.0 /cm): ", {}, false,
												[&mediums](const double& value) { return value >= 0.0; });
		if (!s4) {
			return false;
		}

		auto [s5, g] = read_in_single<double>(in, "  Anisotropy factor g (0.0 - 1.0): ", {}, false,
											  [&mediums](const double& value) { return value >= 0.0 && value <= 1.0; });
		if (!s5) {
			return false;
		}
	...
	std::for_each(mediums.begin(), mediums.end(), [&](Layer& mediums) { out.push_back(mediums); });

	return true;
}

bool ReaderCin::read_output([[maybe_unused]] std::istream& in, std::string& out) {
...
bool ReaderCin::read_layers(std::istream& in, RunParams& params, vec1<Layer>& out) {
	auto make_layer = [&](std::size_t i, double z0, double z1) -> Layer {
		auto [success, name] = read_in_single<std::string>(
			in, {}, "Invalid or duplicate medium name", false, [&params](const std::string& name) {
				return !name.empty()
					   && std::ranges::none_of(params.mediums, [&](const Layer& l) { return l.name == name; });
			});
		if (!success) {
	...
	auto [success, num_layers] = read_in_single<int>(in, "  Total number of layers: ", "Invalid layer number", false,
													 [](const int& num_layers) { return num_layers >= 1; });
	if (!success) {
		return false;
	}
	...
	for (std::size_t i = 0; i < static_cast<std::size_t>(num_layers); ++i) {
		...
		if (i == 0) {
			...
			auto [layer_success, thickness] =
				read_in_single<double>(in, "  Thickness of layer (thickness > 0.0 cm): ", {}, false,
									   [](const double& thickness) { return thickness > 0.0; });
			if (!layer_success) {
				return false;
			}
	...
	std::for_each(layers.begin(), layers.end(), [&](Layer& layer) { out.push_back(layer); });

	std::cout << std::endl;
	return true;
}

bool ReaderCin::read_source(std::istream& input, RunParams& params, LightSource& out) {
...
bool ReaderCin::read_grid(std::istream& in, Grid& out) {
	using namespace std;
	...
	auto [s1, dz, dr, dt] =
		read_in<double, double, double>(in, prompt, error, false, [](const std::tuple<double, double, double>& t) {
	...
	auto [s2, nz, nr, nt, na] =
		read_in<int, int, int, int>(in, prompt, error, false, [](const std::tuple<int, int, int, int>& t) {
...
bool ReaderCin::ReadGridSpacing(std::istream& in, Grid& out) {
	using namespace std;
...
bool ReaderCin::ReadGridSize(std::istream& in, Grid& out) {
	using namespace std;
...
bool ReaderCin::read_record(std::istream& input, RunParams& params, Record& out) {
	...
	if (!Reader::read_record(input, params, out)) {
		return false;
	}
...
bool ReaderCin::read_target(std::istream& input, RunParams& params, Target& out, bool add) {
	if (add) {
	...
	if (Reader::read_target(input, params, out, add)) {
		return false;
	}
...
bool ReaderCin::read_weight(std::istream& input, double& out) {
	std::string prompt = "Input weight threshold (0 <= W < 1.0, 0.0001 recommended)";
	auto [success, weight] = read_in_single<double>(input, prompt, {}, false,
													[](const double& weight) { return weight >= 0.0 && weight < 1.0; });
	if (!success) {
		return false;
	}

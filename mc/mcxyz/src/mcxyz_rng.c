...
void get_random_generator_info(char* state_info, size_t buffer_size) {
	...
	snprintf(state_info, buffer_size, "Random generator: PCG32 (state=0x%016llx, inc=0x%016llx)",
			 (unsigned long long)g_rng_state.state, (unsigned long long)g_rng_state.inc);
}

...
static McxyzErrorCode write_fluence_data(const SimulationConfig* config) {
	...
	printf("Saved: %s (%.1f MB)\n", filename, (config->grid.total_voxels * sizeof(float)) / (1024.0 * 1024.0));
...
McxyzErrorCode validate_simulation_config(const SimulationConfig* config) {
	...
	if (config->grid.size_x <= 0 || config->grid.size_x > 10000 || config->grid.size_y <= 0
		|| config->grid.size_y > 10000 || config->grid.size_z <= 0 || config->grid.size_z > 10000) {
		fprintf(stderr, "Error: Invalid grid dimensions %d×%d×%d (must be 1-10000)\n", config->grid.size_x,
				config->grid.size_y, config->grid.size_z);
		return MCXYZ_ERROR_INVALID_PARAMETER;
	}

	// Validate voxel spacing
	if (config->grid.spacing_x <= 0.0F || config->grid.spacing_x > 1.0F || config->grid.spacing_y <= 0.0F
		|| config->grid.spacing_y > 1.0F || config->grid.spacing_z <= 0.0F || config->grid.spacing_z > 1.0F) {
		fprintf(stderr, "Error: Invalid voxel spacing %.6f×%.6f×%.6f cm (must be 0-1)\n", config->grid.spacing_x,
				config->grid.spacing_y, config->grid.spacing_z);
		return MCXYZ_ERROR_INVALID_PARAMETER;
	}

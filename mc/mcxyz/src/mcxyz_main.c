...
static void print_simulation_summary(const SimulationConfig* config, const ProgramConfig* prog_config) {
    ...
    printf("Total volume: %.3f × %.3f × %.3f cm\n",
           config->grid.volume_x, config->grid.volume_y, config->grid.volume_z);
...
static int handle_error(McxyzErrorCode error_code, const char* context, SimulationConfig* sim_config) {
    ...
    switch (error_code) {
        ...
        case MCXYZ_ERROR_MEMORY_ALLOCATION:
            error_message = "Memory allocation failed";
            break;
            
        case MCXYZ_ERROR_FILE_NOT_FOUND:
            error_message = "Input file not found";
            break;
            
        case MCXYZ_ERROR_FILE_FORMAT:
            error_message = "Invalid file format";
            break;
            
        case MCXYZ_ERROR_INVALID_PARAMETER:
            error_message = "Invalid parameter";
            break;
            
        case MCXYZ_ERROR_SIMULATION_FAILURE:
            error_message = "Simulation failed";
            break;
            
        case MCXYZ_ERROR_OUTPUT_WRITE:
            error_message = "Output file write failed";
            break;

 ...
 * Major modifications in version 2.1 include:
 *	  . C17 standard compliance for modern compiler compatibility.
 *    . Replace legacy random number generator with more modern and faster algorithm.
 *    . Enhance memory safety with secure allocation and string handling.
 *    . Add buffer overflow protection and format string validation.
 *    . Improve performance through compiler optimization hints.
 ****
...
/* Use high-precision mathematical constants */
#ifndef M_PI
...
#define BUF_SIZE   256          /* Buffer size for fgets operations. */
#define SMALL_BUF  80           /* Small buffer for temporary strings. */
#define C_LIGHT    0.0299792458 /* Speed of light in vacuum [cm/ps]. */
#define ONE_OVER_C 33.35640952  /* 1/C [ps/cm]. */

#define SIGN(x) ((x) >= 0 ? 1 : -1)

/********************************* Stuctures *****************************/

enum source_type { pencil, isotropic };
...
typedef struct {
	...
	double w;          /* weight. */
	bool alive;        /* 1/0 if photon is alive/terminated. */
...
typedef struct {
	...
	enum source_type source_type; /* beam type. */
...
/**************************************************************************
 *	Modern array structure to replace archaic pointer arithmetic.
 ****/
/**************************************************************************
...
RngState* CreateRngState(long seed);
void FreeRngState(RngState* rng);
double RandomGenModern(RngState* restrict rng);

/**************************************************************************
 *	Hybrid array structures for internal management.
 *  These maintain metadata while returning raw pointers for compatibility.
 ****/
typedef struct {
	double* raw_ptr;  // The pointer returned to caller
	size_t size;      // Allocated size
	ptrdiff_t offset; // Index offset for access
} Array1D;

typedef struct {
	double** raw_ptr;     // The pointer returned to caller
	double** data;        // Row pointers
	size_t rows;          // Number of rows
	size_t cols;          // Number of columns
	ptrdiff_t row_offset; // Row offset
	ptrdiff_t col_offset; // Column offset
} Array2D;

typedef struct {
	double*** raw_ptr;          // The pointer returned to caller
	double*** data;             // 3D pointer structure
	size_t dim1, dim2, dim3;    // Dimensions
	ptrdiff_t off1, off2, off3; // Offsets
} Array3D;

/**************************************************************************
 *	Modernized array allocation function prototypes.
 *  These provide better safety while maintaining legacy compatibility.
 ****/
...
void FreeArray1D(const double* array);
void FreeArray2D(double** array);
void FreeArray3D(double*** array);

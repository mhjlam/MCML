...
static const int GAUSSIAN_LIMIT_FACTOR = 4; /* Ignore values beyond this factor * radius */

/*************************************************************************
 *  The optimized point in the ith grid.
 ****/
#define RBEST(i) ((((i) + 0.5) + 1.0 / (12.0 * ((i) + 0.5))) * dr)
 ...
 ****/
void GetArbitraryBeam(BeamStru *restrict Beam_Ptr) {
	FILE *restrict file;
	char Fname[STRLEN];
	char buf[STRLEN];
	...
	if (fgets(input_buffer, sizeof(input_buffer), stdin) != NULL) {
		...
		strncpy(Fname, input_buffer, sizeof(Fname) - 1);
	...
	// Safely copy filename with length check
	size_t fname_len = strlen(Fname);
	size_t max_copy = sizeof(Beam_Ptr->pro_fname) - 1;
	if (fname_len < max_copy) {
		strcpy(Beam_Ptr->pro_fname, Fname);
	}
	else {
		memcpy(Beam_Ptr->pro_fname, Fname, max_copy);
		Beam_Ptr->pro_fname[max_copy] = '\0';
	}
	...
	/* allocate arrays. */
	if ((Beam_Ptr->r = AllocArray1D(0, Beam_Ptr->np - 1)) == NULL
		|| (Beam_Ptr->pd = AllocArray1D(0, Beam_Ptr->np - 1)) == NULL) {
...
void LaserBeam(BeamStru *Beam_Ptr) {
	...
	switch (toupper(cmd_str[0])) {
		case 'F': GetFlatBeam(Beam_Ptr); break;
		case 'G': GetGaussianBeam(Beam_Ptr); break;
		case 'A': GetArbitraryBeam(Beam_Ptr); break;
		case 'Q': break;
		default:; puts("Unsupported beam type."); break;
	}
}
 ...
 ****/
double M_rxInterp(const double r2, ConvStru *restrict Conv_Ptr) {
	const double dr = Conv_Ptr->dr;
	const short ix = Conv_Ptr->ix;
	const short nr = Conv_Ptr->nr;

	if (__builtin_expect(nr <= 2, 0)) { /* Unlikely case */
		return Conv_Ptr->M_rx[0][ix];
	}

	/* find the low index according to r2. */
	const double ir2_dbl = r2 / dr;
	short ir2lo;

	if (__builtin_expect(ir2_dbl <= 1.0, 0)) { /* Boundary condition - less likely */
		ir2lo = 0;
	}
	else {
		ir2lo = 0.5 * (ir2_dbl + sqrt((ir2_dbl * ir2_dbl) - (1.0 / 3.0)) - 0.5);
		ir2lo = MIN(ir2lo, nr - 3);
	}

	const double r2lo = RBEST(ir2lo);
	const double M_lo = Conv_Ptr->M_rx[ir2lo][ix];
	const double M_hi = Conv_Ptr->M_rx[ir2lo + 1][ix];
	const double M_at_r2 = M_lo + ((M_hi - M_lo) * (r2 - r2lo) / (RBEST(ir2lo + 1) - r2lo));

	return MAX(0.0, M_at_r2);
}
...
float ArbIntegrand(float theta2, ConvStru *Conv_Ptr) {
	double r = Conv_Ptr->r;
	double rc = Conv_Ptr->rc;

	return (ArbSource(sqrt((r * r) + (rc * rc) - (2 * r * rc * cosf(theta2))), Conv_Ptr));
}
...
float M_rxIntegrand(float r2, ConvStru *Conv_Ptr) {
	...
	if ((link = SearchNode(Conv_Ptr->tree, r2))) { /* integrand in tree. */
		...
		if (beam.type == flat) {
			...
			integrand = qtrap(ArbIntegrand, 0, 2 * M_PI, Conv_Ptr);
		}
...
double GaussIntegration(float (*Func)(float, ConvStru *), ConvStru *Conv_Ptr) {
	double R = Conv_Ptr->beam.R;
	double b_max = (Conv_Ptr->nr - 0.5) * Conv_Ptr->dr;
	double a = MAX(0.0, Conv_Ptr->rc - GAUSSIAN_LIMIT_FACTOR * R);
	double b = MIN(b_max, Conv_Ptr->rc + GAUSSIAN_LIMIT_FACTOR * R);

	if (a >= b) {
		return 0.0;
	}
	return qtrap(Func, (float)a, (float)b, Conv_Ptr);
}
...
void ConvM_rx(ConvStru *Conv_Ptr, double **M_rxc /* convolved array. */) {
	...
	for (irc = 0; irc < Conv_Ptr->nrc; irc++) {
		...
		if (Conv_Ptr->beam.type == flat) {
			factor = (rc <= R) ? 0.5 / M_PI : 0;
		}
		else if (Conv_Ptr->beam.type == gaussian) {
			factor = 0.5 / M_PI * exp(-2 * rc * rc / (R * R));
		}
		else {
		...
		for (ix = 0; ix < Conv_Ptr->nxc; ix++) {
			...
			if (Conv_Ptr->beam.type == flat) {
				M_rxc[irc][ix] = 2 * P / (R * R) * (FlatIntegration(M_rxIntegrand, Conv_Ptr) + ballistic);
			}
			else if (Conv_Ptr->beam.type == gaussian) {
				M_rxc[irc][ix] = 4 * P / (R * R) * (GaussIntegration(M_rxIntegrand, Conv_Ptr) + ballistic);
			}
			else { /* Arbitrary. */

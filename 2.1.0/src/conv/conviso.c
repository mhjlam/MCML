...
void SortAnIsoLine(PairList *PairHeadPtr, struct XY Pmax) {
	char sorted = 0;
	PairList this;
	PairList last;               /* this=the pair being compared w/ the
								  * next one. */
...
void IsoPlot(double **Z, /* the 2D array Z[i][j]. */
	...
	struct XY pmin;
	struct XY pmax; /* the xy positions of the min & max. */
	double zmin;
	double zmax;
	IsoList isos;
	char fname[128];

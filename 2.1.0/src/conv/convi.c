...
enum { FIRSTINTERACTIONS = 1 };

/**************************************************************************
 *  Kill the ith char (counting from 0), push the following chars forward
 *  by one.
 ****/
...
bool CheckCharQ(char *Str) {
	bool found = false; /* found bad char. */
	size_t sl = strlen(Str);
	size_t i = 0;
...
bool CommentLineQ(char *Buf) {
	size_t spn = strspn(Buf, " \t");   /* length spanned by space or tab chars. */
 ...
 ****/
char *FindDataLine(FILE *restrict File_Ptr) {
	static char buf[STRLEN];

	do { /* skip space or comment lines. */
		if (fgets(buf, sizeof(buf) - 1, File_Ptr) == NULL) {
			printf("Incomplete data.\n");
			buf[0] = '\0';
			return buf; // Return consistent pointer even on error
		}
		CheckCharQ(buf);
	}
...
/**************************************************************************
 *  Get the filename and open it for reading, retry until the file can be
 *  opened, or a '.' is input.
 ****/
FILE *GetFile(char *restrict Fname) {
	FILE *file = NULL;
	...
	do {
		...
		if (len >= STRLEN) {
			printf("Filename too long! Maximum %d characters.\n", STRLEN - 1);
			continue;
		}

		strncpy(Fname, input_buffer, STRLEN - 1);
		Fname[STRLEN - 1] = '\0'; // Ensure null termination
 ...
 ****/
bool CheckFileVersionQ(char *Buf, char *Version) {
...
/**************************************************************************
 *  Allocate the arrays in OutStru for one run, and array elements are
 *  automatically initialized to zeros.
 *
 *  Remember that the indices for Rd_r[], Td_r[], & A_rz[][iz] start from
 *  -1 storing the collimated responses.
 ****/
...
bool ReadMediumListQ(FILE *Fp, InStru *In_Ptr) {
	...
	In_Ptr->mediumlist = (LayerStru *)malloc((unsigned)(In_Ptr->num_media) * sizeof(LayerStru));
...
bool ReadFnameFormatQ(FILE *restrict Fp, InStru *restrict In_Ptr) {
	...
	char temp_fname[STRLEN];
	if (sscanf(buf, "%255s %c", temp_fname, &(In_Ptr->out_fformat)) != 2) {
		printf("Reading file name and format.\n");
		return false;
	}

	// Safely copy the filename with length check
	size_t fname_len = strlen(temp_fname);
	size_t max_copy = sizeof(In_Ptr->out_fname) - 1;
	if (fname_len < max_copy) {
		strcpy(In_Ptr->out_fname, temp_fname);
	}
	else {
		memcpy(In_Ptr->out_fname, temp_fname, max_copy);
		In_Ptr->out_fname[max_copy] = '\0';
	}
...
bool ReadLayerSpecsQ(FILE *Fp, InStru *In_Ptr) {
	...
	short i = 0;
	double z = 0.0;
	double thick; /* z coordinate of the current layer. */
	int num_layers;
	int index;
	long file_pos;
 ...
 *  If the z is on an interface between layers, the returned index will
 *  point to the upper layer.
 *  Index 0 is the top ambient medium and index num_layers+1 is the bottom
 *  one.
 ****/
...
bool ReadStartPQ(FILE *Fp, InStru *In_Ptr) {
	...
	if ((In_Ptr->type == isotropic) && (sz == 0.0)) {
...
void FreeData(InStru *In_Ptr, OutStru *Out_Ptr) {
	/* Don't need the dimension variables anymore since modern functions handle cleanup automatically */
	(void)In_Ptr; /* Suppress unused parameter warning */
...
void IOAb_zt(FILE *Fp, short Nz, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Ab[z][t]. [1/(cm ps)]", "# Ab[0][0], [0][1],..[0][nt-1]", "# Ab[1][0], [1][1],..[1][nt-1]", "# ...",
				"# Ab[nz-1][0], [nz-1][1],..[nz-1][nt-1]", "Ab_zt");
	}
	else {
...
void IOA_zt(FILE *Fp, short Nz, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# A[z][t]. [1/(cm ps)]", "# A[0][0], [0][1],..[0][nt-1]", "# A[1][0], [1][1],..[1][nt-1]", "# ...",
				"# A[nz-1][0], [nz-1][1],..[nz-1][nt-1]", "A_zt");
	}
	else {
...
void IORd_rat(FILE *Fp, short Nr, short Na, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Rd[r][a][t]. [1/(cm2 sr ps)]", "# Rd[0][0][0], [0][0][1],..[0][0][nt-1]",
...
void IORd_ra(FILE *Fp, short Nr, short Na, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Rd[r][angle]. [1/(cm2 sr)].", "# Rd[0][0], [0][1],..[0][na-1]", "# Rd[1][0], [1][1],..[1][na-1]",
...
void IORd_rt(FILE *Fp, short Nr, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Rd[r][t]. [1/(cm2 ps)]", "# Rd[0][0], [0][1],..[0][nt-1]", "# Rd[0][0], [0][1],..[0][nt-1]", "# ...",
				"# Rd[nr-1][0], [nr-1][1],..[nr-1][nt-1]", "Rd_rt");
	}
	else {
		FindDataLine(Fp);
	}
...
void IORd_at(FILE *Fp, short Na, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Rd[a][t]. [1/(sr ps)]", "# Rd[0][0], [0][1],..[0][nt-1]", "# Rd[1][0], [1][1],..[1][nt-1]", "# ...",
				"# Rd[na-1][0], [na-1][1],..[na-1][nt-1]", "Rd_at");
	}
	else {
		FindDataLine(Fp);
	}
...
void IOTd_rat(FILE *Fp, short Nr, short Na, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Td[r][a][t]. [1/(cm2 sr ps)]", "# Td[0][0][0], [0][0][1],..[0][0][nt-1]",
...
void IOTd_ra(FILE *Fp, short Nr, short Na, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Td[r][angle]. [1/(cm2 sr)].", "# Td[0][0], [0][1],..[0][na-1]", "# Td[1][0], [1][1],..[1][na-1]",
				"# ...", "# Td[nr-1][0], [nr-1][1],..[nr-1][na-1]", "Td_ra");
	}
	else {
		FindDataLine(Fp);
	}
...
void IOTd_rt(FILE *Fp, short Nr, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Td[r][t]. [1/(cm2 ps)]", "# Td[0][0], [0][1],..[0][nt-1]", "# Td[0][0], [0][1],..[0][nt-1]", "# ...",
				"# Td[nr-1][0], [nr-1][1],..[nr-1][nt-1]", "Td_rt");
	}
	else {
		FindDataLine(Fp);
	}
...
void IOTd_at(FILE *Fp, short Na, short Nt, OutStru *Out_Ptr, char Mode) {
	...
	if (Mode == 1) {
		fprintf(Fp, "%s\n%s\n%s\n%s\n%s\n%s\n", /* flag. */
				"# Td[a][t]. [1/(sr ps)]", "# Td[0][0], [0][1],..[0][nt-1]", "# Td[1][0], [1][1],..[1][nt-1]", "# ...",
				"# Td[na-1][0], [na-1][1],..[na-1][nt-1]", "Td_at");
	}
	else {
		FindDataLine(Fp);
	}
 ...
 ****/
bool ReadOutMCV2(FILE *Fp, InStru *In_Ptr, OutStru *Out_Ptr, char Mode) {
 ...
 ****/
bool FoundKeyWord(char *LineBuf, char *Key) {
	char *sub_str;
	bool found = 0;
 ...
 ****/
bool SeekKey(FILE *File_Ptr, char *Key) {
	char *line;
 ...
 *
 ****/
bool ReadLayerParam(FILE *File_Ptr, short Num_Layers, LayerStru **Layers_PP) {
	char *buf;
	short i = 0;
	double d;
	double n;
	...
	/* layer 0 and layer Num_Layers + 1 are for ambient. */
	*Layers_PP = (LayerStru *)malloc((unsigned)(Num_Layers + 2) * sizeof(LayerStru));
	if (!(*Layers_PP)) {
 ...
 *  simulations.
 ****/
bool ReadInParam(FILE *File_Ptr, InStru *In_Ptr) {
	char *buf;
 ...
 ****/
bool ReadRATV1(FILE *File_Ptr, OutStru *Out_Ptr) {
	char *buf;
...
/**************************************************************************
 ****/
bool ReadRd_ra(FILE *File_Ptr, short Nr, short Na, OutStru *Out_Ptr) {
	short ir;
	short ia;
...
/**************************************************************************
 ****/
bool ReadTd_ra(FILE *File_Ptr, short Nr, short Na, OutStru *Out_Ptr) {
	short ir;
	short ia;
 ...
 ****/
bool ReadOutMCV1(FILE *File_Ptr, InStru *In_Ptr, OutStru *Out_Ptr) {

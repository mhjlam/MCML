...
FILE *GetWriteFile(char *Ext) {
	...
	do {
		...
		if (fgets(fname, sizeof(fname), stdin) != NULL) {
			...
			// Skip if input is too long or empty
			if (len == 0) {
				continue;
			}
		}
		...
		if (file != NULL) { // File exists
			...
			do {
				if (fgets(fmode, sizeof(fmode), stdin) != NULL) {
					// Remove trailing newline
					size_t len = strlen(fmode);
					if (len > 0 && fmode[len - 1] == '\n') {
						fmode[len - 1] = '\0';
					}
				}
				else {
			...
			while (strlen(fmode) == 0); // Avoid null line
			fclose(file);
			file = NULL;
		}
	}
	...
	// Safely copy the filename to Ext if it fits
	if (Ext && strlen(fname) < STRLEN) {
		strncpy(Ext, fname, STRLEN - 1);
		Ext[STRLEN - 1] = '\0';
	}
 ...
 *  OPTIMIZED: Added restrict qualifiers and loop optimizations
 ****/
void A_rzFromA_rzt(InStru *restrict In_Ptr, OutStru *restrict Out_Ptr) {
	const short nr = In_Ptr->nr;
	const short nz = In_Ptr->nz;
	const short nt = In_Ptr->nt;
	const double dt = In_Ptr->dt;

	/* Cache-friendly loop ordering and manual loop unrolling where beneficial */
	for (short ir = 0; ir < nr; ir++) {
		double *restrict a_rz_row = Out_Ptr->A_rz[ir];
		double **restrict a_rzt_row = Out_Ptr->A_rzt[ir];

		for (short iz = 0; iz < nz; iz++) {
			double sum = 0.0;
			const double *restrict a_rzt_elem = a_rzt_row[iz];

			/* Manual unrolling for better performance */
			short it = 0;
			for (; it < (nt & ~3); it += 4) {
				sum += a_rzt_elem[it] + a_rzt_elem[it + 1] + a_rzt_elem[it + 2] + a_rzt_elem[it + 3];
			}
 ...
 *  OPTIMIZED: Added restrict qualifiers and vectorization hints
 ****/
void Ab_zFromAb_zt(InStru *restrict In_Ptr, OutStru *restrict Out_Ptr) {
	const short nz = In_Ptr->nz;
	const short nt = In_Ptr->nt;
	const double dt = In_Ptr->dt;
	...
	for (short iz = 0; iz < nz; iz++) {
		double sum = 0.0;
		const double *restrict ab_zt_row = Out_Ptr->Ab_zt[iz];

		/* Manual unrolling for better performance */
		short it = 0;
		for (; it < (nt & ~3); it += 4) {
			sum += ab_zt_row[it] + ab_zt_row[it + 1] + ab_zt_row[it + 2] + ab_zt_row[it + 3];
		}
		/* Handle remaining elements */
		for (; it < nt; it++) {
			sum += ab_zt_row[it];
...
void A_zFromA_rzt(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (iz = 0; iz < nz; iz++) {
		...
		for (it = 0; it < nt; it++) {
			for (ir = 0; ir < nr; ir++) {
				Out_Ptr->A_z[iz] += Out_Ptr->A_rzt[ir][iz][it] * dt * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
...
void A_zFromA_rz(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (iz = 0; iz < nz; iz++) {
		Out_Ptr->A_z[iz] = 0.0;
		for (ir = 0; ir < nr; ir++) {
			Out_Ptr->A_z[iz] += Out_Ptr->A_rz[ir][iz] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
		}
...
void A_tFromA_rzt(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		...
		for (iz = 0; iz < nz; iz++) {
			for (ir = 0; ir < nr; ir++) {
				Out_Ptr->A_t[it] += Out_Ptr->A_rzt[ir][iz][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr * dz;
			}
...
void Rd_rFromRd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ir = 0; ir < nr; ir++) {
		...
		for (ia = 0; ia < na; ia++) {
			for (it = 0; it < nt; it++) {
				Out_Ptr->Rd_r[ir] += Out_Ptr->Rd_rat[ir][ia][it] * dt * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
}
...
void Rd_rFromRd_ra(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ir = 0; ir < nr; ir++) {
		Out_Ptr->Rd_r[ir] = 0.0;
		for (ia = 0; ia < na; ia++) {
			Out_Ptr->Rd_r[ir] += Out_Ptr->Rd_ra[ir][ia] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
		}
	}
}
...
void Rd_aFromRd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ia = 0; ia < na; ia++) {
		...
		for (ir = 0; ir < nr; ir++) {
			for (it = 0; it < nt; it++) {
				Out_Ptr->Rd_a[ia] += Out_Ptr->Rd_rat[ir][ia][it] * dt * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
		}
	}
}
...
void Rd_aFromRd_ra(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ia = 0; ia < na; ia++) {
		Out_Ptr->Rd_a[ia] = 0.0;
		for (ir = 0; ir < nr; ir++) {
			Out_Ptr->Rd_a[ia] += Out_Ptr->Rd_ra[ir][ia] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
		}
	}
}
...
void Rd_tFromRd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		...
		for (ir = 0; ir < nr; ir++) {
			for (ia = 0; ia < na; ia++) {
				Out_Ptr->Rd_t[it] += Out_Ptr->Rd_rat[ir][ia][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr * M_PI
									 * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
}
...
void Rd_tFromRd_rt(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		Out_Ptr->Rd_t[it] = 0.0;
		for (ir = 0; ir < nr; ir++) {
			Out_Ptr->Rd_t[it] += Out_Ptr->Rd_rt[ir][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
		}
	}
}
...
void Rd_tFromRd_at(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		Out_Ptr->Rd_t[it] = 0.0;
		for (ia = 0; ia < na; ia++) {
			Out_Ptr->Rd_t[it] += Out_Ptr->Rd_at[ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
		}
	}
}
...
void Td_rFromTd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ir = 0; ir < nr; ir++) {
		...
		for (ia = 0; ia < na; ia++) {
			for (it = 0; it < nt; it++) {
				Out_Ptr->Td_r[ir] += Out_Ptr->Td_rat[ir][ia][it] * dt * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
}
...
void Td_rFromTd_ra(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ir = 0; ir < nr; ir++) {
		Out_Ptr->Td_r[ir] = 0.0;
		for (ia = 0; ia < na; ia++) {
			Out_Ptr->Td_r[ir] += Out_Ptr->Td_ra[ir][ia] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
		}
	}
}
...
void Td_aFromTd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ia = 0; ia < na; ia++) {
		...
		for (ir = 0; ir < nr; ir++) {
			for (it = 0; it < nt; it++) {
				Out_Ptr->Td_a[ia] += Out_Ptr->Td_rat[ir][ia][it] * dt * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
		}
	}
}
...
void Td_aFromTd_ra(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (ia = 0; ia < na; ia++) {
		Out_Ptr->Td_a[ia] = 0.0;
		for (ir = 0; ir < nr; ir++) {
			Out_Ptr->Td_a[ia] += Out_Ptr->Td_ra[ir][ia] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
		}
	}
}
...
void Td_tFromTd_rat(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		...
		for (ir = 0; ir < nr; ir++) {
			for (ia = 0; ia < na; ia++) {
				Out_Ptr->Td_t[it] += Out_Ptr->Td_rat[ir][ia][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr * M_PI
									 * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
...
void Td_tFromTd_rt(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		Out_Ptr->Td_t[it] = 0.0;
		for (ir = 0; ir < nr; ir++) {
			Out_Ptr->Td_t[it] += Out_Ptr->Td_rt[ir][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
		}
	}
}
...
void Td_tFromTd_at(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	for (it = 0; it < nt; it++) {
		Out_Ptr->Td_t[it] = 0.0;
		for (ia = 0; ia < na; ia++) {
			Out_Ptr->Td_t[it] += Out_Ptr->Td_at[ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
		}
	}
}
...
void ExtractRAT(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr) {
	...
	strncpy(fname, "RAT", sizeof(fname) - 1);
...
double IzToMua(short iz, InStru *In_Ptr) {
	double tz;
	double bz; /* top and bottom z of ith box. */
	double mua;
	...
	if (((tz < thelayer.z0) && (bz > thelayer.z0)) || ((tz < thelayer.z1) && (bz > thelayer.z1))) {
...
#if IBMPC == 0
	if (mode == 0) {
		strncpy(fname, "Arz@t", STRLEN - 1);
		fname[STRLEN - 1] = '\0';
	}
	if (mode == 1) {
		strncpy(fname, "Frz@t", STRLEN - 1);
		fname[STRLEN - 1] = '\0';
	}
#endif
	if (mode == 2) {
		strncpy(fname, "Arz", STRLEN - 1);
		fname[STRLEN - 1] = '\0';
	}
	if (mode == 3) {
		strncpy(fname, "Frz", STRLEN - 1);
		fname[STRLEN - 1] = '\0';
	}
	if ((file = GetWriteFile(fname)) == NULL) {
		return;
...
void ExtractA_rz_t(InStru *In_Ptr, OutStru *Out_Ptr, char mode) {
	...
	if ((Out_Ptr->A_rz = AllocArray2D(0, nr - 1, 0, nz - 1)) == NULL) {
...
void ExtractConvA_rz_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, nz - 1)) == NULL)
		|| ((Conv_Ptr->Mb_x = AllocArray1D(0, nz - 1)) == NULL)
		|| ((A_rzc = AllocArray2D(0, Conv_Ptr->nrc - 1, 0, nz - 1)) == NULL)) {
...
void ExtractConvA_rz(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.A_rz) {
		if (((Out_Ptr->A_rz = AllocArray2D(0, nr - 1, 0, nz - 1)) == NULL)
			|| ((Out_Ptr->Ab_z = AllocArray1D(0, nz - 1)) == NULL)
			|| ((A_rzc = AllocArray2D(0, Conv_Ptr->nrc - 1, 0, nz - 1)) == NULL)) {
...
void ExtractA_z_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.A_rzt) {
		for (iz = 0; iz < nz - 1; iz++) {
			temp = 0.0;
			for (ir = 0; ir < nr; ir++) {
				temp += Out_Ptr->A_rzt[ir][iz][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
...
void ExtractA_z(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.A_z) {
		if ((Out_Ptr->A_z = AllocArray1D(0, nz - 1)) == NULL) {
...
void ExtractConvA_t_r_z(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	Conv_Ptr->drc = 2 * pr;
	Conv_Ptr->nrc = 1;
	if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, nt - 1)) == NULL)
		|| ((Conv_Ptr->Mb_x = AllocArray1D(0, nz - 1)) == NULL)
		|| ((A_rtc = AllocArray2D(0, Conv_Ptr->nrc - 1, 0, nt - 1)) == NULL)) {
...
void ExtractA_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.A_t) {
		if ((Out_Ptr->A_t = AllocArray1D(0, nt - 1)) == NULL) {
...
void ExtractA_l(InStru *In_Ptr, OutStru *Out_Ptr) {
	...
	if ((A_layer = AllocArray1D(1, num_layers)) == NULL) {
...
void ExtractRd_r_a_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		ConvResolution(Conv_Ptr);
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Rd_rxc = AllocArray2D(0, Conv_Ptr->nrc - 1, 0, 0)) == NULL)) {
...
void ExtractRd_r_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
		|| ((mode == 1) && (Rd_rxc = AllocArray2D(0, Conv_Ptr->nrc - 1, 0, 0)) == NULL)) {
...
void ExtractRd_r_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.Rd_rat) {
		for (ir = 0; ir < nr - 1; ir++) {
			Conv_Ptr->M_rx[ir][0] = 0.0;
			for (ia = 0; ia < na; ia++) {
				Conv_Ptr->M_rx[ir][0] += Out_Ptr->Rd_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
...
void ExtractRd_r(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Rd_r) {
		if ((Out_Ptr->Rd_r = AllocArray1D(0, nr - 1)) == NULL) {
...
void ExtractRd_a_r_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		Conv_Ptr->nrc = 1;
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Rd_rac = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
...
void ExtractRd_a_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.Rd_rat) {
		for (ia = 0; ia < na; ia++) {
			temp = 0.0;
			for (ir = 0; ir < nr - 1; ir++) {
				temp += Out_Ptr->Rd_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
			}
			if (mode == 1) {
				temp *= Conv_Ptr->beam.P;
			}
...
void ExtractRd_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Rd_a) {
		if ((Out_Ptr->Rd_a = AllocArray1D(0, na - 1)) == NULL) {
...
void ExtractRd_t_r_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		Conv_Ptr->nrc = 1;
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Rd_rtc = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
			printf("No enough memory to convolve Rd_a@r@t.\n");
			return;
		}
		...
		if (In_Ptr->record.Rd_rat) {
			for (ir = 0; ir < nr - 1; ir++) {
				for (it = 0; it < nt; it++) {
					Conv_Ptr->M_rx[ir][it] = 0.0;
					for (ia = 0; ia < na; ia++) {
						Conv_Ptr->M_rx[ir][ia] += Out_Ptr->Rd_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
					}
				}
			}
		}
...
void ExtractRd_t_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.Rd_rat) {
		for (it = 0; it < nt; it++) {
			temp = 0.0;
			for (ir = 0; ir < nr - 1; ir++) {
				temp += Out_Ptr->Rd_rat[ir][ia][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
			if (mode == 1) {
				temp *= Conv_Ptr->beam.P;
			}
...
void ExtractRd_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Rd_t) {
		if ((Out_Ptr->Rd_t = AllocArray1D(0, nt - 1)) == NULL) {
...
void ExtractTd_r_a_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		ConvResolution(Conv_Ptr);
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Td_rxc = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
...
void ExtractTd_r_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
		|| ((mode == 1) && (Td_rxc = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
...
void ExtractTd_r_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.Td_rat) {
		for (ir = 0; ir < nr - 1; ir++) {
			Conv_Ptr->M_rx[ir][0] = 0.0;
			for (ia = 0; ia < na; ia++) {
				Conv_Ptr->M_rx[ir][0] += Out_Ptr->Td_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
			}
		}
	}
...
void ExtractTd_r(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Td_r) {
		if ((Out_Ptr->Td_r = AllocArray1D(0, nr - 1)) == NULL) {
...
void ExtractTd_a_r_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		Conv_Ptr->nrc = 1;
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Td_rac = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
...
void ExtractTd_a_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (In_Ptr->record.Td_rat) {
		for (ia = 0; ia < na; ia++) {
			temp = 0.0;
			for (ir = 0; ir < nr - 1; ir++) {
				temp += Out_Ptr->Td_rat[ir][ia][it] * 2.0 * M_PI * (ir + 0.5) * dr * dr;
			}
			if (mode == 1) {
				temp *= Conv_Ptr->beam.P;
			}
...
void ExtractTd_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Td_a) {
		if ((Out_Ptr->Td_a = AllocArray1D(0, na - 1)) == NULL) {
...
void ExtractTd_t_r_a(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (mode == 0) {
		...
		Conv_Ptr->nrc = 1;
		if (((Conv_Ptr->M_rx = AllocArray2D(0, nr - 1, 0, 0)) == NULL)
			|| ((Td_rtc = AllocArray2D(0, nr - 1, 0, 0)) == NULL)) {
			printf("No enough memory to convolve Td_a@r@t.\n");
			return;
		}
	...
	if (mode == 0) {
		if (In_Ptr->record.Td_rat) {
			for (it = 0; it < nt; it++) {
				temp = 0.0;
				for (ia = 0; ia < na; ia++) {
					temp += Out_Ptr->Td_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
				}
				fprintf(file, "%-12.4E\t%-12.4E\n", (it + 0.5) * dt, temp);
			}
		}
		...
		if (In_Ptr->record.Td_rat) {
			for (ir = 0; ir < nr - 1; ir++) {
				for (it = 0; it < nt; it++) {
					Conv_Ptr->M_rx[ir][it] = 0.0;
					for (ia = 0; ia < na; ia++) {
						Conv_Ptr->M_rx[ir][ia] += Out_Ptr->Td_rat[ir][ia][it] * M_PI * sin(2.0 * (ia + 0.5) * da) * da;
					}
				}
			}
		}
...
void ExtractTd_t(InStru *In_Ptr, OutStru *Out_Ptr, ConvStru *Conv_Ptr, char mode) {
	...
	if (!In_Ptr->record.Td_t) {
		if ((Out_Ptr->Td_t = AllocArray1D(0, nt - 1)) == NULL) {
...
char *ToUpperString(char *string) {
	...
	// Calculate length once and store it
